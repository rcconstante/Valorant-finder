  ...
  return (
    <main className="pt-24 pb-32 px-6 max-w-7xl mx-auto">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
        {/* CREATE LOBBY */}
        <Link to="/create-lobby" className="group relative h-80 overflow-hidden cut-corner bg-surface-container-low border-l-4 border-primary-container transition-all hover:shadow-[0_0_30px_rgba(255,82,93,0.2)]">
          <div
          ...
          <div className="relative h-full flex flex-col justify-end p-8 text-left">
            <span className="font-label text-xs font-bold text-primary uppercase tracking-[0.3em] mb-2">
              SYS_INIT_SEQUENCE
            </span>
            <h2 className="font-headline text-5xl font-black text-white uppercase leading-none mb-4">
              CREATE
              <br />
              <span className="text-primary-container">LOBBY</span>
            </h2>
            <p className="font-body text-sm text-on-surface-variant max-w-xs mb-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        ...
        <Link
          to="/find-lobby"
          className="group relative h-80 overflow-hidden cut-corner bg-surface-container-low border-l-4 border-tertiary transition-all hover:shadow-[0_0_30px_rgba(96,220,176,0.2)]"
        >
          <div
            className="absolute inset-0 opacity-20 grayscale group-hover:grayscale-0 transition-all duration-700 bg-cover bg-center"
            style={{
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-background via-transparent to-transparent" />
          <div className="relative h-full flex flex-col justify-end p-8 text-left">
            <span className="font-label text-xs font-bold text-tertiary uppercase tracking-[0.3em] mb-2">
      ...
      {/* Live Feed Section */}
      <section>
        <div className="flex items-end justify-between mb-8 border-l-4 border-primary-container pl-4">
          <div>
            <span className="font-label text-[10px] text-primary uppercase tracking-[0.4em]">
              Satellite Uplink Active
            </span>
            <h3 className="font-headline text-2xl font-black text-[#ece8e1] uppercase tracking-tighter">
              LIVE LOBBY FEED PREVIEW
            </h3>
          </div>
        ...
        {/* Bento Grid Lobby Feed */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-1">
          {previewLobbies.map((lobby) => (
            <div
              key={lobby.name}
              className={`${lobby.bgClass} p-6 flex flex-col justify-between min-h-[160px] hover:bg-surface-container transition-colors cursor-pointer group`}
            >
              <div className="flex justify-between items-start">
                ...
                <div className="bg-tertiary-container/20 px-2 py-0.5 border border-tertiary/30">
              ...
              <div className="flex items-center justify-between mt-8">
                <div className="flex gap-4">
                  <div className="flex flex-col">
                    ...
                    <span className="font-headline text-xs font-bold text-white uppercase">{lobby.mode}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="font-label text-[9px] text-on-surface-variant uppercase">Map</span>
                    <span className="font-headline text-xs font-bold text-white uppercase">{lobby.map}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end">
                  ...
                  <span className="font-headline text-2xl font-black text-white">
                    {lobby.slotsNeeded}
                    <span className="text-on-surface-variant text-sm font-normal">/{lobby.totalSlots}</span>
                  </span>
                </div>
              </div>
            </div>
